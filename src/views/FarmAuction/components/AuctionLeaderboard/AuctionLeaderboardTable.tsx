...
interface LeaderboardRowProps {
  bidder: Bidder
  morrallaPriceBusd: BigNumber
  isMobile: boolean
}

const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ bidder, morrallaPriceBusd, isMobile }) => {
        ...
        <Flex flexDirection="column" width="100%" justifyContent="flex-end" pr={[null, null, '24px']}>
          ...
          {morrallaPriceBusd.gt(0) ? (
            <Text fontSize="12px" color="textSubtle" textAlign="right">
              ~${getBalanceNumber(amount.times(morrallaPriceBusd)).toLocaleString('en', { maximumFractionDigits: 0 })}
            </Text>
          ) : (
      ...
      <LeaderboardContainer>
        ...
        {bidders.slice(0, visibleBidders).map((bidder) => (
          <LeaderboardRow
            key={bidder.account}
            bidder={bidder}
            morrallaPriceBusd={morrallaPriceBusd}
            isMobile={isMobile}
          />
        ))}
      </LeaderboardContainer>
