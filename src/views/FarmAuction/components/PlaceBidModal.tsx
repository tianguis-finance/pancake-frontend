...
import { Modal, Text, Flex, BalanceInput, Box, Button, PancakeRoundIcon } from '@tianguis-finance/uikit'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient MORRALLA balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% MORRALLA or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      <ExistingInfo>
        <Flex justifyContent="space-between">
          ...
          <Text>{t('%num% MORRALLA', { num: getBalanceNumber(amount).toLocaleString() })}</Text>
        </Flex>
        <Flex justifyContent="space-between">
      ...
      <InnerContent>
        <Flex justifyContent="space-between" alignItems="center" pb="8px">
          ...
          <Flex>
            ...
            <Text bold>MORRALLA</Text>
          </Flex>
        </Flex>
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% MORRALLA or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
